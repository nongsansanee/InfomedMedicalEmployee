...
if ($_GET["chk"]=='d'){
	...
	/********************Send Data To WEB MED  by call API addEmp (sansanee 010762)***********************************/
		$url = 'https://www.si.mahidol.ac.th/department/Medicine/home/api/delEmp.asp';
		$data = "action=1&sapid=".$empid."&userin=".$userin;
		//echo "DATA=".$data."<br>";
		//msgBox($data);
		$options = array(
			'http' => array(
...
?>
  <tr class="<?=$classname?>">
	...
	<td align="center">
	  ...
	  <a href="viewemp.php?chk=d&empflag=<?=$empflag?>&empid=<?=$row[0]?>&worktype=<?=$worktype?>&positiontype=<?=$positiontype?>&workposition=<?=$workposition?>&workunit=<?=$workunit?>&follow=<?=$follow?>&txtsearch=<?=$txtsearch?>&userin=<?=$userin?>" onClick="return confirmbox();"><img src="../image/del.gif"  border="0">ยกเลิก</a> 
